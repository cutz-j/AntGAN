...
class Generator(nn.Module):
    # based on resnet
    def __init__(self, transformer, img_size=256, max_conv_dim=512):
        super().__init__()
    ...
    def forward(self, x, t):
        ...
        for block in self.encode:
            x = block(x)
            t = block(t)
        tf_x, x_att_list  = self.transformer(x)
        tf_t, t_att_list = self.transformer(t)
        for block in self.decode:
            x = block(x, tf_t)
        return self.to_rgb(x), tf_x, tf_t
...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Transformer
        x = x.view([x.size(0), x.size(1), -1]).transpose(1, 2) # (bs, 512, 16*16) --> (bs, 256, 512)
        enc_outputs, enc_self_attns = self.encoder(x)
        return enc_outputs, enc_self_attns
...
def build_model(args):
    ...
    generator = Generator(transformer, args.img_size, args.max_conv_dim)
